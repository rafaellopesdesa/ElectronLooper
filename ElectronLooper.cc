...
// PassAllWP2012Cuts
  ...
  TBranch* lowfbrem = redTree->Branch("lowfbrem", &dielectron_mass_lowfbrem, "dielectron_mass_lowfbrem/F");
  TBranch* onecluster  = redTree->Branch("onecluster", &dielectron_mass_onecluster, "dielectron_mass_onecluster/F");
  TBranch* golden = redTree->Branch("golden", &dielectron_mass_golden, "dielectron_mass_golden/F");
  ...
  while (!fileReader.eof()) {
    ...
    for (Long64_t ientry = 0; ientry < nentries; ientry++) {
      ...
      lowfbrem->Fill((els_p4().at(0)+els_p4().at(1)).mass(), evt_scale1fb()); 
       }

           if (els_nSeed().at(0) == 0 && els_nSeed().at(1) == 0) {
      onecluster->Fill((els_p4().at(0)+els_p4().at(1)).mass(), evt_scale1fb()); 
            }

      if (els_class().at(0) == GOLDEN && els_class().at(1) == GOLDEN) {
      golden->Fill((els_p4().at(0)+els_p4().at(1)).mass(), evt_scale1fb()); 
      }
